...
Several projects emerged in the open-source community around mid-2023, and OpenAI introduced their [Structured Output API](https://openai.com/index/introducing-structured-outputs-in-the-api/) in August 2024. These are based on a technique called **constrained decoding**. 
...
The model must generate a JSON from the image of an invoice. Code is available at [https://github.com/AnOtterGithubUser/structured_generation_with_small_models](https://github.com/AnOtterGithubUser/structured_generation_with_small_models).
...
Now at least we know when something is wrong, even without a ground-truth (which is usually not available) and set aside invoices that cannot be processed automatically.              
